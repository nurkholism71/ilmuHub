...
import { Star, BookOpen, BarChart2, LayoutGrid, List, Heart, ChevronLeft, ChevronRight } from 'lucide-react';

export const allClassesCatalog = [
  {
    id: 1,
...
export default function ClassesGrid({
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 bg-white p-3.5 rounded-2xl border border-gray-200/80 shadow-2xs">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            Temukan kelas yang sesuai dengan minat dan tujuan belajarmu. (1,230 classes found)
          </p>
        </div>
      ...
      {sortedCourses.length === 0 ? (
        ...
        <div className={viewMode === 'grid' 
          ...
          {sortedCourses.map((item) => (
            ...
            >
              {/* Image with Tag Badge & Heart button */}
              <div className={`relative overflow-hidden bg-gray-100 ${
                ...
                {item.badge && (
                  <div className="absolute top-2 left-2">
                    <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold shadow-xs ${item.badgeColor || 'bg-[#114B44] text-white'}`}>
                      {item.badge}
                    </span>
                  </div>
                )}
              ...
              {/* Details */}
              <div className={`flex-1 flex flex-col justify-between ${viewMode === 'grid' ? 'p-3.5' : ''}`}>
                <div>
      ...
      <div className="mt-8 pt-4 border-t border-gray-200/80 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-gray-500">
        <div>
          Showing 1–12 of 1,230 classes
        </div>

        <div className="flex items-center gap-1">
          <button 
            onClick={() => setCurrentPage(Math.max(1, currentPage - 1))}
            className="w-7 h-7 rounded-lg border border-gray-200 flex items-center justify-center hover:bg-gray-50 cursor-pointer disabled:opacity-40"
          ...
          <button 
            onClick={() => setCurrentPage(Math.min(103, currentPage + 1))}
