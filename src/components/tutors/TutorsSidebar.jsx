...
export default function TutorsSidebar({
  selectedSubjects,
  onToggleSubject,
  selectedLevels,
  onToggleLevel,
  teachingType,
  onChangeTeachingType,
  priceFilter,
  onChangePriceFilter,
  priceRange,
  onChangePriceRange,
  ...
  const subjectsList = [
    { id: 'nahwu', label: 'Nahwu', count: 124 },
    { id: 'sharaf', label: 'Sharaf', count: 98 },
    { id: 'arabic', label: 'Arabic Language', count: 210 },
    { id: 'quran', label: 'Quran', count: 76 },
    { id: 'hadith', label: 'Hadith', count: 64 },
    { id: 'fiqh', label: 'Fiqh', count: 52 },
    { id: 'islamic_studies', label: 'Islamic Studies', count: 120 },
    { id: 'university', label: 'University Subjects', count: 188 },
    { id: 'english', label: 'English', count: 96 },
    { id: 'mathematics', label: 'Mathematics', count: 84 },
  ];

  const levelsList = [
    { id: 'Beginner', label: 'Beginner', count: 320 },
    { id: 'Intermediate', label: 'Intermediate', count: 410 },
    { id: 'Advanced', label: 'Advanced', count: 180 },
  ];

  const teachingTypes = ['All Types', 'Live Class', 'Recorded Class', 'Hybrid'];
  ...
  return (
    <aside className="w-full bg-white rounded-2xl border border-gray-200/90 p-4 sm:p-5 space-y-5 text-xs text-gray-700 shadow-2xs">
      ...
      {/* Teaching Type */}
      <div className="space-y-2 pt-3 border-t border-gray-100">
        <h4 className="font-bold text-gray-900 text-xs">Teaching Type</h4>
        <div className="space-y-1.5">
          {teachingTypes.map((type) => (
            <label key={type} className="flex items-center gap-2 text-gray-600 hover:text-gray-900 cursor-pointer select-none">
              <input
                type="radio"
                name="teachingType"
                checked={teachingType === type}
                onChange={() => onChangeTeachingType(type)}
